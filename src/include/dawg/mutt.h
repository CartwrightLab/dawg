...
public:
	...
	std::uint64_t rand_uint64() { return gen.rand_uint64(); }
	// returns random 32-bit number
	std::uint32_t rand_uint32() { return gen.rand_uint32(); }
	// returns random 32-bit number with max n-1
	std::uint32_t rand_uint32(std::uint32_t n) {
		return static_cast<std::uint32_t>(rand_real()*n);
	}
	...
	inline std::uint32_t rand_geometric_q(double q) {
		return 1+static_cast<std::uint32_t>(rand_exp(q));
	}

	// random zeta with slope z
	std::uint32_t rand_zeta(double z);
...
private:
	generator gen;
	std::vector<std::uint32_t> _seed;

	static const std::uint64_t ek[256];
...
inline std::uint32_t create_random_seed() {
	std::uint32_t v = static_cast<std::uint32_t>(getpid());
	v += ((v << 15) + (v >> 3)) + 0x6ba658b3; // Spread 5-decimal PID over 32-bit number
	v^=(v<<17); v^=(v>>13); v^=(v<<5);
	v += static_cast<std::uint32_t>(time(nullptr));
	v^=(v<<17); v^=(v>>13); v^=(v<<5);
