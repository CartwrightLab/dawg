...
public:
    ...
    bool create(It1 first_n, It1 last_n, It2 first_r, It2 last_r,
	    ...
	    throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("maximum indel\
		size is out of range."));
        }
        if(first_n == last_n) {
	    throw dawg::dawg_error_t(dawg_error::indel_model_no_type);
        }
        ...
        for(It2 it=first_r;it!=last_r;++it) {
            if(*it < 0.0) {
		throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("invalid indel\
		    model; rate" + boost::lexical_cast<std::string>(*it) + " must be positive."));
	    }
            total_rate += *it;
        }
        if(total_rate == 0.0) {
        ...
        for(It2 it=first_r;it!=last_r;++it) {
            ...
            case 6: // lavalette model
                ...
                break;
            default:
		throw dawg::dawg_error_t(dawg_error::model_no_name, \
		    std::string(std::string(itn->c_str()) + " (invalid indel model)."));
            };
            if(!okay) {
		throw dawg::dawg_error_t(dawg_error::creation_fail, std::string("indel model."));
	    }
...
private:
    ...
    inline bool create_geo(double f, It &first, It last, unsigned int max_size,
        ...
        if(first == last) {
	    throw dawg::dawg_error_t(dawg_error::param_missing, std::string("geo requires\
		1 parameter (invalid indel model)."));
	}
        double p = *first++;
        if(p >= 1.0)
            p = 1.0/p;
        if(p <= 0.0) {
	    throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("geo parameter " +\
		std::to_string(p) + "must be positive (invalid indel model)."));
	}
        double d = p;
    ...
    inline bool create_zeta(double f, It &first, It last, unsigned int max_size,
                           std::vector<double> &mix_dist) {
        if(first == last) {
	    throw dawg::dawg_error_t(dawg_error::param_missing, std::string("zeta requires\
		1 parameter (invalid indel model)."));
	}
        double z = *first++;
        if(z <= 1.0) {
	    throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("zeta parameter" +\
		std::to_string(z) + " must be >1 (invalid indel model)."));
	}
        double zz = zeta(z);
        for(boost::uint32_t n=1; n <= max_size; ++n) {
    ...
    inline bool create_yule(double f, It &first, It last, unsigned int max_size,
                           std::vector<double> &mix_dist) {
        if(first == last) {
	    throw dawg::dawg_error_t(dawg_error::param_missing, std::string("yule requires\
		1 parameter (invalid indel model)."));
	}
        double z = *first++;
        if(z <= 1.0) {
	    throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("yule parameter" +\
		std::to_string(z) + " must be >1 (invalid indel model)."));
	}
        for(boost::uint32_t n=1; n <= max_size; ++n) {
    ...
    inline bool create_lavalette(double f, It &first, It last, unsigned int max_size,
                           std::vector<double> &mix_dist) {
        if(first == last) {
	    throw dawg::dawg_error_t(dawg_error::param_missing, std::string("lavalette requires\
		2 parameters (invalid indel model)."));
	}
        double z = *first++;
        if(first == last) {
	    throw dawg::dawg_error_t(dawg_error::param_missing, std::string("lavalette requires\
        ...
        double dm = *first++; 
        if(z <= 1.0) {
	    throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("lavalette slope" +\
		std::to_string(z) + " must be >1 (invalid indel model)."));
	}
        if(dm <= 1.0) {
	    throw dawg::dawg_error_t(dawg_error::invalid_value, std::string("lavalette max" +\
		std::to_string(dm) + " must be >1 (invalid indel model)."));
	}
    ...
    inline bool create_user(double f, It &first, It last, unsigned int max_size,
        ...
        It it = first;
        if(first == last) {
	    throw dawg::dawg_error_t(dawg_error::param_missing, std::string("no parameters\
		for user model (invalid indel model)."));
	}
        // sum up parameters
